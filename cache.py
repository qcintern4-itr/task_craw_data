...
def save_xml_response_table6(encounter_id, json_text):
    cache_dir = "container_xml_table6"
    if not os.path.exists(cache_dir):
        os.makedirs(cache_dir)
...
def load_xml_response_table6(encounter_id):
    cache_file = os.path.join("container_xml_table6", f"labs_{encounter_id}.json")
    if os.path.exists(cache_file):
        with open(cache_file, "r", encoding="utf-8") as f:
            return f.read()
    return None
